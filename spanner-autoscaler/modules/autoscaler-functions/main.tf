...
resource "google_pubsub_topic" "scaler_topic" {
  name = "scaler-topic-${random_id.suffix.hex}"
}
...
resource "google_storage_bucket" "bucket_gcf_source" {
  name                        = "${var.project_id}-spanner-autoscaler-gcf-source"
...
resource "google_cloudfunctions_function" "poller_function" {
  name                = "tf-poller-function-${random_id.suffix.hex}"
  project             = var.project_id
  region              = var.region
...
resource "google_cloudfunctions_function" "scaler_function" {
  name                = "tf-scaler-function-${random_id.suffix.hex}"
  project             = var.project_id
  region              = var.region
  ingress_settings    = "ALLOW_INTERNAL_AND_GCLB"
  available_memory_mb = "256"
